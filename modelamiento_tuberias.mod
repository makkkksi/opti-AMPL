set N;  # nodos
set E dimen 2; #tuberias posibles
set D; # desaladoras
set P; # poblados
set M; # mineras
...
# cada consumidor debe recibir exactamente su demanda neta
subject to DemandaConsumidor{n in P union M}:
    sum{(i,n) in E} y[i,n] - sum{(n,j) in E} y[n,j] = G[n];

# nodos de paso: lo que entra sale
subject to Transito{n in N diff (D union P union M)}:
    sum{(i,n) in E} y[i,n] = sum{(n,j) in E} y[n,j];

# no hay flujo si no se construye
